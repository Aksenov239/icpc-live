...
public abstract class Widget {
    ...
    void drawTeamPane(Graphics2D g, TeamInfo team, int x, int y, int width, double state) {
        Color color = team.rank <= 4 ? GOLD_COLOR : team.rank <= 8 ? SILVER_COLOR : team.rank <= 12 ? BRONZE_COLOR : ACCENT_COLOR;
        if (team.solved == 0) color = ACCENT_COLOR;
        int height = (int) (width * 0.1);
        g.setFont(Font.decode("Open Sans Italic " + (int) (height * 0.7)));
        drawTextInRect(g, "" + Math.max(team.rank, 1), x, y, (int) (width * RANK_WIDTH), height, POSITION_CENTER, color, Color.WHITE, state);
        x += (int) (width * (RANK_WIDTH + SPLIT));
        drawTextInRect(g, team.shortName, x, y, (int) (width * NAME_WIDTH), height, POSITION_LEFT, MAIN_COLOR, Color.WHITE, state);
        x += (int) (width * (NAME_WIDTH + SPLIT));
        drawTextInRect(g, "" + team.solved, x, y, (int) (width * SOLVED_WIDTH), height, POSITION_CENTER, ADDITIONAL_COLOR, Color.WHITE, state);
        x += (int) (width * (SOLVED_WIDTH + SPLIT));
        drawTextInRect(g, "" + team.penalty, x, y, (int) (width * PENALTY_WIDTH), height, POSITION_CENTER, ADDITIONAL_COLOR, Color.WHITE, state);
    }
